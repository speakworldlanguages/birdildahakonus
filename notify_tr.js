...
function insertData() {
  set(ref(db,"tr/"+index.toUTCString()),{"Token":tokenToBeSaved}).then(()=>{console.log("token is saved");}).catch((error)=>{console.log("couldn't save token: "+error);});
}
  ...
  Notification.requestPermission().then(permission=>{
    if (permission == "granted") { // User clicks/touches [ALLOW]
      ...
      getToken(messaging, {vapidKey:"B"+brokenVapidKey}).then((currentToken) => {
        ...
        reg.showNotification('Yeni dersler',
        { body: "hazır olunca bildirim alacaksın", badge: "user_interface/images/push_notification/badge.png", icon: "user_interface/homescreen_icon/icon_for_pwa_tr.png", image: "user_interface/images/push_notification/notification_image.jpg" });
